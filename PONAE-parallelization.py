...
Parallelization and automation of the run of the PONAE-code.py script for
...
simulacion_param = [1000000, 100, 2024] # MC, n_consumers, seed
folder_name = "prob_nJ"
...
def run_scripts(num_instancias, tipo_simula, valores_J, valores_q, valores_simulaciones, folder_arg):
    ...
    if tipo_simula == "homog":
        val_uniforme = [0.75, 1.0]  #a and b, Uniform distribution
    elif tipo_simula == "heterog":
        val_uniforme = [0.00, 1.0]  #a and b, Uniform distribution
    else:
        val_uniforme = [] 
    ...
    def run_pyfile(params, carpeta):
        comando = f"python PONAE-code.py --param1 {params['param1']} --param2 {params['param2']} --param_uni1 {params['param_uni1']} --param_uni2 {params['param_uni2']} --simu_p_1 {params['simu_p_1']} --simu_p_2 {params['simu_p_2']} --simu_p_3 {params['simu_p_3']} --foldername {folder_arg} --output {carpeta}"
    ...
    with ThreadPoolExecutor(max_workers=num_instancias) as executor:
        for i, params in enumerate(parametros):
            ...
            executor.submit(run_pyfile, params, carpeta)
